...
from data import buffs as buff_data
from data import enemies, gear
from engine import actions, create_player, wsdist
from engine.equipment_rules import apply_ear_slot_rules, apply_weapon_slot_rules, is_right_ear_only
...
from integrations.profile_builder import (
...
from persistence.result_history import ResultHistory
from persistence.simulation_cache import SimulationCache, canonical_json, source_fingerprint
from presentation import fancy_plot
...
CACHE_SOURCE_HASH = source_fingerprint([
        ...
        "equipment_rules.py", "actions.py", "attack_round_model.py", "create_player.py", "wsdist.py",
        "get_hit_rate.py", "get_ma_rate.py", "get_fstr.py",
        "get_pdif.py", "get_phys_damage.py", "get_tp.py",
        "nuking.py", "get_dint_m_v.py", "get_delay_timing.py",
    )
] + [
    DATA_DIR / name for name in (
        "gear.py", "weaponskill_info.py", "weapon_bonus.py", "enemies.py", "buffs.py",
    )
])
SLOTS = (
...
def _normalized_gearset_payload(gearset: dict, main_job: str, sub_job: str,
                                master_level: int) -> dict[str, dict]:
    """Return the same legal equipment view used by ``create_player``.

    Quick Look requests can be keyed before constructing a player, but the
    key must still account for weapon/ammo compatibility and right-ear rules.
    Applying the shared normalization here keeps UI requests and overnight
    warm tasks on the same identity.
    """
    normalized = _gearset_payload(gearset)
    apply_weapon_slot_rules(normalized, main_job, sub_job, master_level)
    apply_ear_slot_rules(normalized)
    return normalized
...
def _quick_cache_request(action: str, gearset: dict, *, main_job: str,
    ...
    request = {
        "action": action,
        "gearset": _normalized_gearset_payload(
            gearset, main_job, sub_job, master_level
        ),
        "main_job": main_job,
        "sub_job": sub_job,
...
class GearIconProvider:
    ...
    def __init__(self):
        ...
        try:
            with (DATA_DIR / "item_list.csv").open(encoding="utf-8", newline="") as stream:
    ...
    def plot_icon_sources(self) -> tuple[Path, ...]:
        ...
        return self._roots() + (ASSET_DIR / "icons32.zip",)

    def _archive_icon(self, item_id: int) -> QIcon:
        archive_path = ASSET_DIR / "icons32.zip"
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._quick_lookup_cache_limit = 4096
        self._quick_lookup_persistent_keys: set[str] = set()
        self._hydrate_quick_lookup_cache()
    ...
    def _build_ui(self):
        ...
        self.setStyleSheet(stylesheet.replace(
            "__CHECKMARK_GOLD__", (ASSET_DIR / "checkmark-gold.svg").as_posix()
        ))
    ...
    def _remember_quick_lookup(self, cache_key: str, payload: dict):
        """Promote one exact Quick Look payload into the session LRU."""
        if not isinstance(payload, dict) or "text" not in payload or "output" not in payload:
            return
        self._quick_lookup_cache[cache_key] = payload
    ...
    def _hydrate_quick_lookup_cache(self):
        """Load recent persistent Quick Look rows into the fast session cache."""
        if not self.cache_enabled:
            return
        for cache_key, record in self.simulation_cache.load_recent(
            "quick-look", self._quick_lookup_cache_limit
        ).items():
            self._remember_quick_lookup(cache_key, record.get("payload", {}))
            self._quick_lookup_persistent_keys.add(cache_key)

    def _persistent_quick_lookup(self, cache_key: str) -> dict | None:
        """Return one persistent Quick Look payload and promote it to RAM."""
        if not self.cache_enabled:
            return None
        record = self.simulation_cache.get(cache_key, "quick-look")
        if record is None:
            return None
        payload = record.get("payload")
        if not isinstance(payload, dict) or "text" not in payload or "output" not in payload:
            return None
        self._remember_quick_lookup(cache_key, payload)
        self._quick_lookup_persistent_keys.add(cache_key)
        return payload
    ...
    def evaluate(self, action: str):
        try:
            ...
            request_enemy = _report_enemy(context["enemy"], context["debuffs"])
            request = _quick_cache_request(
                action, self.quick_set.items,
                main_job=context["main_job"], sub_job=context["sub_job"],
                master_level=context["master_level"], buffs=context["buffs"],
                abilities=context["abilities"], enemy=request_enemy.stats,
            ...
            persistent_hit = cache_key in self._quick_lookup_persistent_keys
            if cached is None:
                cached = self._persistent_quick_lookup(cache_key)
                persistent_hit = cached is not None
                if not persistent_hit:
                    self._quick_lookup_persistent_keys.discard(cache_key)

            # A hit skips the expensive action calculation, but the current
            # Quick Look panel still needs the player/enemy objects for its
            # detailed stat cards and comparison graphs.
            player = create_player.create_player(
                context["main_job"], context["sub_job"], context["master_level"],
                gearset=self.quick_set.items,
                buffs=context["buffs"], abilities=context["abilities"],
            )
            enemy = request_enemy
            if cached is not None:
                self._quick_lookup_cache.move_to_end(cache_key)
                ...
                message = (
                    "Quick Look restored from persistent cache"
                    if persistent_hit else "Quick Look restored from this session's cache"
                )
            ...
            if action == "ws":
                ...
                self._start_quick_dps_comparison(player, enemy)
            payload = {"text": text, "output": _json_value(output)}
            self._remember_quick_lookup(cache_key, {"text": text, "output": output})
            if self.cache_enabled:
                self.simulation_cache.put(
                    cache_key, "quick-look", payload,
                    time.monotonic() - started_at,
                )
                self._quick_lookup_persistent_keys.add(cache_key)
        except Exception as error:
