...
from engine.equipment_rules import apply_weapon_slot_rules, has_conditional_set_effect, ranged_attack_ready, ranged_pair_compatible
...
def format_bgwiki(ws_name, tp, player, best_metric):
    ...
    item_list = np.loadtxt(
        Path(__file__).resolve().parents[1] / "data" / "item_list.csv",
        unpack=False, dtype=str, delimiter=';', usecols=(1, 2), skiprows=1,
    )
