...
class SimulationCache:
    ...
    def load_recent(self, kind: str, limit: int | None = 256) -> dict[str, dict]:
        """Load reusable rows for startup hydration without updating hit counts.

        The normal ``get`` path intentionally updates LRU metadata, but doing
        that once for every row during application startup would turn cache
        hydration into a write-heavy operation.  Callers receive the same
        payload envelope returned by ``get`` and may place the rows into a
        faster in-memory cache.
        """
        now = time.time()
        try:
            query = (
                "SELECT cache_key, created_at, runtime_seconds, payload "
                "FROM simulation_results "
                "WHERE kind = ? AND source_hash = ? "
                "ORDER BY accessed_at DESC"
            )
            parameters: tuple = (str(kind), self.source_hash)
            ...
            expired = []
            with self._connection() as connection:
                for key, created_at, runtime_seconds, encoded in connection.execute(query, parameters):
                    if now - float(created_at) > self.max_age_seconds:
                        expired.append((key,))
                        continue
                    try:
