...
from engine import create_player
from data import gear
from app.qt_gui_main import MainWindow
...
class SimulationCacheTests(unittest.TestCase):
    ...
    def test_load_recent_hydrates_rows_without_changing_hit_counts(self):
        first_key = self.cache.key_for("quick-look", {"index": 1})
        second_key = self.cache.key_for("quick-look", {"index": 2})
        self.assertTrue(self.cache.put(first_key, "quick-look", {"value": 1}, 0.1))
        self.assertTrue(self.cache.put(second_key, "quick-look", {"value": 2}, 0.2))

        loaded = self.cache.load_recent("quick-look", limit=1)
        self.assertEqual(len(loaded), 1)
        self.assertIn(second_key, loaded)
        self.assertEqual(loaded[second_key]["payload"], {"value": 2})
        self.assertEqual(self.cache.summary()["hits"], 0)
