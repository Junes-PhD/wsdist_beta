...
from engine.create_player import create_player
from data.gear import Sagitta, all_jobs
...
class FancyPlotTests(unittest.TestCase):
    ...
    def test_missing_gear_icon_does_not_abort_distribution(self):
        ...
        with (
            ...
            fancy_plot.plot_final(
                ...
                "Victory Smite",
                icons_path=(),
                items_file=Path(__file__).resolve().parents[1] / "data" / "item_list.csv",
            )
    ...
    def test_ico_icon_is_supported(self):
        ...
        with zipfile.ZipFile(Path(__file__).resolve().parents[1] / "assets" / "icons32.zip") as source:
